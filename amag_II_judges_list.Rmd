...
library(readr) # Package use to load CSV files
```
...
```{r}
# Save as a dataframe
judges_list <- as.data.frame(df_subset)

# Create CSV output 
write.csv(x=judges_list, file="amag_ii_judges_list.csv", row.names=FALSE)

# Inspect to ensure encoding is accurate
df_review <- read_csv('amag_ii_judges_list.csv')
```
