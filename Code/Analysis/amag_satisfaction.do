...
outreg2 using satisfactionref.xls, replace stats(coef pval ) level(95) addtext(mean, `mean') label keep(socratic  i.en_Course) addnote(The table shows the impact of courses on satisfaction which is an scale that goes from 1 to 5 The regression controls for age, gender and position of the participant, second model motivated reasoner at bs and third interaction of motivated reasoner at endline . Standard errors are clustered at the course level.) bdec(3)
...
reg meansat socratic $controls3 ,  cluster(Course)  
	    outreg2 using satisfactionref.xls, append stats(coef pval ) level(95)  addtext(mean, `mean') label keep(socratic i.en_Course) bdec(3)
...
outreg2 using grades.xls, replace stats(coef pval ) level(95) addtext(mean, `mean') label keep(socratic_treated socratic_actual  attendance i.en_Course) addnote(The table shows the impact of courses on grade which is variable that ranges from 0 to 20. The regression controls for age, gender and position of the participant, second model motivated reasoner at bs and third interaction of motivated reasoner at endline . Standard errors are clustered at the course level.) bdec(3)
  

reg grade socratic_actual   $controls2 ,  cluster(Course)
outreg2 using grades.xls, append stats(coef pval ) level(95) addtext(mean, `mean') label keep(socratic_treated socratic_actual attendance i.en_Course) addnote(The table shows the impact of courses on grade which is variable that ranges from 0 to 20. The regression controls for age, gender and position of the participant, second model motivated reasoner at bs and third interaction of motivated reasoner at endline . Standard errors are clustered at the course level.) bdec(3)
...
reg grade attendance  socratic_treated $controls3 ,  cluster(Course)  
	    outreg2 using grades.xls, append stats(coef pval ) level(95)  addtext(mean, `mean') label keep(socratic_treated attendance  i.en_Course) bdec(3)
