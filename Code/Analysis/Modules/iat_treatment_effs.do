...
***********/
	...
	-----*/
	orth_out position_* course_* gender_1 Age_rounded using "$tables\iat_selection.texx" if iat_option_take_or_not==1, by(en_iat_player_skipped) pcompare stars sheet(selection_iat_takeup) sheetreplace

	orth_out position_* course_* gender_1 Age_rounded using "$tables\iat_selection.texx" if iat_feedback_option_or_not==1, by(en_ignore_feedback) pcompare stars sheet(selection_iat_feedback) sheetreplace
	...
	**sets of variables
	...
	foreach y in $iat_outcomes {
		...
		if(`reg_counter'==1){ // creating outreg file
			outreg2 using "$tables\iat_option_take_or_not.tex", replace stats(coef pval ) level(95) addtext("Controls", "No", "Mean of dep var. ctrl.", `mean') label keep(iat_option_take_or_not) nocons nor2
			} 
		else{ // storing reg output in outreg file
			outreg2 using "$tables\iat_option_take_or_not.tex", append stats(coef pval ) level(95) addtext("Controls", "No", "Mean of dep var. ctrl.", `mean') label keep(iat_option_take_or_not) nocons nor2
		} 
		...
		outreg2 using "$tables\iat_option_take_or_not.tex", append stats(coef pval ) level(95) addtext("Controls", "Yes", "Mean of dep var. ctrl.", `mean') label keep(iat_option_take_or_not) nocons nor2 // storing r
	...
	foreach y in $iat_outcomes_nf{
		...
		if(`reg_counter'==1){ // creating outreg file
			outreg2 using "$tables\iat_feedback_option_or_not.tex", replace stats(coef pval ) level(95) addtext("Controls", "No", "Mean of dep var. ctrl.", `mean') label keep(iat_feedback_option_or_not) nocons nor2
			} 
		else{ // storing reg output in outreg file
			outreg2 using "$tables\iat_feedback_option_or_not.tex", append stats(coef pval ) level(95) addtext("Controls", "No", "Mean of dep var. ctrl.", `mean') label keep(iat_feedback_option_or_not) nocons nor2
		}
		...
		outreg2 using "$tables\iat_feedback_option_or_not.tex", append stats(coef pval ) level(95) addtext("Controls", "Yes", "Mean of dep var. ctrl.", `mean') label keep(iat_feedback_option_or_not) nocons nor2 // storing r
		local reg_counter = `reg_counter' + 1
	}
	...
	foreach y in $iat_outcomes{
		...
		if(`reg_counter'==1){ // creating outreg file
			outreg2 using "$tables\iat_interaction.tex", replace stats(coef pval ) level(95) addtext("Controls", "No", "Mean of dep var. ctrl.", `mean') label keep(iat_option_take_or_not iat_feedback_option_or_not iat_interaction) nocons nor2
			} 
		else{ // storing reg output in outreg file
			outreg2 using "$tables\iat_interaction.tex", append stats(coef pval ) level(95) addtext("Controls", "No", "Mean of dep var. ctrl.", `mean') label keep(iat_option_take_or_not iat_feedback_option_or_not iat_interaction) nocons nor2
		}
		...
		outreg2 using "$tables\iat_interaction.tex", append stats(coef pval ) level(95) addtext("Controls", "Yes", "Mean of dep var. ctrl.", `mean') label keep(iat_option_take_or_not iat_feedback_option_or_not iat_interaction) nocons nor2 // storing r
		local reg_counter = `reg_counter' + 1
	}
